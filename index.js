...
function renderTooltip(b) {
    const summaryHtml = `<ul id="tt-summary">${b.summary.map(s => `<li>${s}</li>`).join("")}</ul>`;
    tooltip
        .style("display", "block")
        .html(`<span id="tt-close">×</span><h3>${b.name}</h3><div class="dates">${b.dates}</div>${summaryHtml}<a id="tt-readmore" href="info/index.html?id=${b.id}">Read more →</a>`);
}
